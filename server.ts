...
} from "./src/services/precisionHunterEngine.js";
import {
  formatPrecisionHunterTelegramMessage,
  formatHaramiAiTelegramMessage,
  formatKhatarnakJugaadTelegramMessage,
  formatWarRoomTelegramMessage,
} from "./src/services/centralTelegramDispatcher.js";
import {
  ActiveCentralSetup,
} from "./src/services/centralSignalManager.js";
import {
...
async function startServer() {
  ...
  const TELEGRAM_DELIVERY_LOGS_FILE = path.join(process.cwd(), ".telegram_delivery_logs.json");
  ...
  interface TelegramBotUser {
    ...
    botAccess?: "all" | "khatarnak" | "harami" | "war_room" | "precision_hunter";
  ...
  interface ServerTelegramDeliveryRecord {
    id: string;
    signalId: string;
    engine: string;
    timestampUtc: string;
  ...
  interface ServerTelegramDeliveryFailure {
    timestampUtc: string;
    userId: string;
    reason: string;
  }
  ...
  function loadTelegramDeliveryLogs() {
    try {
      if (fs.existsSync(TELEGRAM_DELIVERY_LOGS_FILE)) {
        const data = JSON.parse(fs.readFileSync(TELEGRAM_DELIVERY_LOGS_FILE, "utf-8"));
        serverDeliveryLogs = data.logs || [];
        serverDeliveryFailures = data.failures || [];
      }
    } catch (e) {
      serverDeliveryLogs = [];
      serverDeliveryFailures = [];
    }
  }

  function saveTelegramDeliveryLogs() {
    try {
      if (serverDeliveryLogs.length > 200) {
        serverDeliveryLogs = serverDeliveryLogs.slice(0, 200);
      }
      if (serverDeliveryFailures.length > 200) {
        serverDeliveryFailures = serverDeliveryFailures.slice(0, 200);
      }
      fs.writeFileSync(
        TELEGRAM_DELIVERY_LOGS_FILE,
        JSON.stringify({ logs: serverDeliveryLogs, failures: serverDeliveryFailures }, null, 2),
        "utf-8"
      );
    } catch (e) {
      console.warn("[TELEGRAM DELIVERY LOGS SAVE ERROR]:", e);
    }
  }
  ...
  async function handleTelegramAdminCallback(cb: any): Promise<void> {
    ...
    if (data.startsWith("adm:bot:toggle:")) {
      ...
      if (botKey === "precision_hunter") {
        ...
        superAdminService.logAction("BOT_TOGGLED", `Precision Hunter AI toggled to ${cfg.precisionHunterEnabled ? "ON" : "OFF"}`, cbUserId);
      } else if (botKey === "harami") {
    ...
    // =========================================================================
    // CENTRAL SIGNAL MANAGER & 4-AI TRADING MANAGEMENT TELEGRAM CONTROLS
    // =========================================================================
    if (data === "adm:csm:menu") {
      const liveGold = fcsMarketService.getLiveTick("XAUUSD")?.price || 4495.50;
      const csmState = centralSignalManager.getState(liveGold);
      const menu = superAdminService.renderCentralSignalManagerMenu(csmState, liveGold);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }
    ...
    if (data.startsWith("adm:ai:toggle:")) {
      const aiKey = data.replace("adm:ai:toggle:", "");
      const cfg = superAdminService.getConfig();
      if (aiKey === "harami") {
        cfg.haramiEnabled = !cfg.haramiEnabled;
        centralSignalManager.setAiSourceEnabled("HARAMI_AI", cfg.haramiEnabled);
        superAdminService.logAction("AI_TOGGLED", `Harami AI toggled to ${cfg.haramiEnabled ? "ON" : "OFF"}`, cbUserId);
      } else if (aiKey === "khatarnak") {
        cfg.khatarnakEnabled = cfg.khatarnakEnabled === false ? true : false;
        centralSignalManager.setAiSourceEnabled("KHATARNAK_JUGAAD", cfg.khatarnakEnabled);
        superAdminService.logAction("AI_TOGGLED", `Khatarnak Jugaad toggled to ${cfg.khatarnakEnabled ? "ON" : "OFF"}`, cbUserId);
      } else if (aiKey === "war_room") {
        cfg.warRoomEnabled = !cfg.warRoomEnabled;
        centralSignalManager.setAiSourceEnabled("WAR_ROOM", cfg.warRoomEnabled);
        superAdminService.logAction("AI_TOGGLED", `War Room toggled to ${cfg.warRoomEnabled ? "ON" : "OFF"}`, cbUserId);
      } else if (aiKey === "precision_hunter") {
        cfg.precisionHunterEnabled = cfg.precisionHunterEnabled === false ? true : false;
        centralSignalManager.setAiSourceEnabled("PRECISION_HUNTER", cfg.precisionHunterEnabled);
    ...
    if (data === "adm:ai:all:on") {
      const cfg = superAdminService.getConfig();
      cfg.haramiEnabled = true;
    ...
    if (data === "adm:ai:all:off") {
      const cfg = superAdminService.getConfig();
      cfg.haramiEnabled = false;
    ...
    if (data === "adm:csm:active") {
      const liveGold = fcsMarketService.getLiveTick("XAUUSD")?.price || 4495.50;
      const csmState = centralSignalManager.getState(liveGold);
      const activeSetup = csmState.activeSetup || (serverActiveTrade ? {
        setupId: serverActiveTrade.signalId || serverActiveTrade.id,
        brainName: serverActiveTrade.isWarRoomUpgraded ? "War Room Supreme" : "Harami AI Master",
        brainEmoji: serverActiveTrade.isWarRoomUpgraded ? "🛡️" : "🤖",
        assetKey: "XAUUSD",
        timeframe: "15M",
        direction: serverActiveTrade.direction,
        lifecycleStatusLabel: serverActiveTrade.status,
        entryZoneLow: Array.isArray(serverActiveTrade.entryZone) ? serverActiveTrade.entryZone[0] : serverActiveTrade.entry - 1,
        entryZoneHigh: Array.isArray(serverActiveTrade.entryZone) ? serverActiveTrade.entryZone[1] : serverActiveTrade.entry + 1,
        preferredEntry: serverActiveTrade.entry,
        protectedSlLevel: serverActiveTrade.sl,
        stopLoss: serverActiveTrade.sl,
        tp1: serverActiveTrade.tp1,
        tp2: serverActiveTrade.tp2,
        tp3: serverActiveTrade.tp3,
        finalTp: serverActiveTrade.tp4 || serverActiveTrade.tp3,
        rrRatioString: "1:3.2",
        setupScore: serverActiveTrade.confidence || 92,
        marketConfidence: 95,
        pnlPips: serverActiveTrade.pnlPips || 0,
        pnlUSD: serverActiveTrade.currentFloatingPnL || 0,
        isBreakeven: serverActiveTrade.sl === serverActiveTrade.entry,
        isTp1Hit: !!serverActiveTrade.tp1Hit,
        isTp2Hit: !!serverActiveTrade.tp2Hit,
        isTp3Hit: !!serverActiveTrade.tp3Hit,
        isFinalTpHit: !!serverActiveTrade.tp4Hit,
      } : null);
      const menu = superAdminService.renderActiveSetupDetailView(activeSetup, liveGold);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:csm:queue") {
      const liveGold = fcsMarketService.getLiveTick("XAUUSD")?.price || 4495.50;
      const csmState = centralSignalManager.getState(liveGold);
      const candidates = Object.values(csmState.candidates || {}).filter(
        (c) => c.competitionStatus === "QUEUED_WAITING" || c.qualityGrade === "STRONG" || c.qualityGrade === "VALID"
      );
      const menu = superAdminService.renderQueuedSetupsView(candidates, csmState.activeSetup);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:csm:cooldown") {
    ...
    if (data === "adm:csm:cd:reset") {
      centralSignalManager.resetCooldown();
      superAdminService.logAction("COOLDOWN_RESET", "Super Admin manually reset post-trade cooldown", cbUserId);
      await answerTelegramCallback(cbId, "🟢 Cooldown reset! All AI models ready.", false);
      const cooldown = centralSignalManager.getCooldown();
      const menu = superAdminService.renderCooldownStatusView(cooldown);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    ...
    if (data === "adm:csm:competition") {
      const liveGold = fcsMarketService.getLiveTick("XAUUSD")?.price || 4495.50;
      const csmState = centralSignalManager.getState(liveGold);
      const menu = superAdminService.renderAiCompetitionView(csmState.candidates, csmState.leaderboard);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:csm:trace") {
      const logs = centralSignalManager.getAuditLogs();
      const menu = superAdminService.renderDecisionTraceView(logs);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data.startsWith("adm:csm:history:")) {
      const filter = data.replace("adm:csm:history:", "") || "ALL";
      const logs = centralSignalManager.getAuditLogs();
      const historyItems = logs
        .filter((l) => l.eventType === "SETUP_ACTIVATED" || l.eventType === "TP_HIT" || l.eventType === "SL_HIT" || l.eventType === "SETUP_CLOSED")
        .filter((l) => (filter === "ALL" ? true : l.selectedBrain === filter))
        .map((l) => ({
          setupId: l.selectedSetupId || "HA-101",
          source: l.selectedBrain || "AI",
          direction: l.direction || "BUY",
          entry: 4495.50,
          outcome: l.eventType === "TP_HIT" ? "TP1" : l.eventType === "SL_HIT" ? "SL" : "ACTIVE",
          pnlPips: l.finalPnlPips || (l.eventType === "TP_HIT" ? 45 : l.eventType === "SL_HIT" ? -18 : 12),
          timeUtc: l.timeFormatted || "Today",
        }));
      const menu = superAdminService.renderSignalHistoryView(filter, historyItems);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:csm:rejected") {
      const logs = centralSignalManager.getAuditLogs();
      const menu = superAdminService.renderRejectedSetupsView(logs);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:csm:market") {
      const goldTick = fcsMarketService.getLiveTick("XAUUSD");
      const goldData = {
        price: goldTick?.price || 4495.50,
        bid: goldTick?.bid || 4495.35,
        ask: goldTick?.ask || 4495.65,
        spread: goldTick ? (goldTick.ask - goldTick.bid) : 0.30,
        high24h: goldTick?.high24h || 4518.20,
        low24h: goldTick?.low24h || 4478.40,
        regime: "INSTITUTIONAL_CONFLUENCE",
        volatility: "HEALTHY_EXPANSION",
      };
      const menu = superAdminService.renderMarketStatusView(goldData);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:csm:trd:be") {
      if (serverActiveTrade) {
        ...
        if (mt5Config.telegramSignalsEnabled) {
          const sigOk = await ensureTradeSignalDispatched(serverActiveTrade);
          if (sigOk) {
            await sendServerTelegramMessage(beMsg, undefined, undefined, `${serverActiveTrade.signalId || serverActiveTrade.id}_BREAKEVEN`);
          }
        }
        ...
        await answerTelegramCallback(cbId, "🔒 SL moved to Break-even", false);
      } else {
        await answerTelegramCallback(cbId, "⚠️ No active trade currently open", false);
      }
      const liveGold = fcsMarketService.getLiveTick("XAUUSD")?.price || 4495.50;
      const csmState = centralSignalManager.getState(liveGold);
      const menu = superAdminService.renderActiveSetupDetailView(csmState.activeSetup, liveGold);
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:csm:trd:secure") {
      if (serverActiveTrade) {
        ...
        superAdminService.logAction("PROFIT_SECURED", `Secured profit alert sent for #${serverActiveTrade.signalId || serverActiveTrade.id}`, cbUserId);
        await answerTelegramCallback(cbId, "🔒 Profit Secured Alert Broadcasted", false);
      } else {
        await answerTelegramCallback(cbId, "⚠️ No active trade currently open", false);
      }
      const liveGold = fcsMarketService.getLiveTick("XAUUSD")?.price || 4495.50;
    ...
    if (data === "adm:precision_hunter:menu") {
      const menu = superAdminService.renderPrecisionHunterControlMenu();
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }

    if (data === "adm:precision_hunter:toggle") {
      const cfg = superAdminService.getConfig();
      cfg.precisionHunterEnabled = cfg.precisionHunterEnabled === false ? true : false;
      centralSignalManager.setAiSourceEnabled("PRECISION_HUNTER", cfg.precisionHunterEnabled);
      superAdminService.saveConfig();
      superAdminService.logAction("PRECISION_HUNTER_TOGGLED", `Precision Hunter AI set to ${cfg.precisionHunterEnabled ? "ON" : "OFF"}`, cbUserId);
      const menu = superAdminService.renderPrecisionHunterControlMenu();
      await editTelegramMessageText(cbChatId, cbMsgId, menu.text, menu.keyboard);
      return;
    }
...
`.trim();
    ...
    // Direct One-Tap Telegram Access Request Handlers (Inline Buttons from New User Alert)
    if (data.startsWith("adm:req:approve:")) {
      const targetUserId = data.replace("adm:req:approve:", "").trim();
      const userKey = Object.keys(telegramUsersStore).find(
        (k) => telegramUsersStore[k].userId === targetUserId || telegramUsersStore[k].chatId === targetUserId
      );
      const user = userKey ? telegramUsersStore[userKey] : null;

      if (!user) {
        await answerTelegramCallback(cbId, "⚠️ User not found in database.", true);
        return;
      }
...
`.trim();
      ...
      const updatedKeyboard: TelegramInlineKeyboard = {
        inline_keyboard: [
          ...
          [
            { text: "👤 View Full Profile", callback_data: `adm:user:view:${targetUserId}` },
            { text: "👥 All Users", callback_data: "adm:users:menu" },
          ],
        ],
      };

      await editTelegramMessageText(cbChatId, cbMsgId, updatedText, updatedKeyboard);
    ...
    if (data.startsWith("adm:req:reject:")) {
      const targetUserId = data.replace("adm:req:reject:", "").trim();
      const userKey = Object.keys(telegramUsersStore).find(
        (k) => telegramUsersStore[k].userId === targetUserId || telegramUsersStore[k].chatId === targetUserId
      );
      const user = userKey ? telegramUsersStore[userKey] : null;
      ...
      superAdminService.logAction(
        "USER_REJECTED_TELEGRAM",
        `Rejected access request for ${user.firstName} (${targetUserId}) via Telegram One-Tap button`,
        cbUserId,
        targetUserId
      );

      await answerTelegramCallback(cbId, `❌ Access request rejected for ${user.firstName}.`, false);

      const updatedText = `
❌ <b>USER ACCESS REJECTED</b>
━━━━━━━━━━━━━━━━━━━
<b>👤 User:</b> <b>${user.firstName || "Trader"} ${user.lastName || ""}</b> (${user.username || "No @username"})
<b>🆔 Telegram ID:</b> <code>${user.userId}</code>
<b>🔒 Status:</b> 🔴 <code>REJECTED</code>
<b>🕒 Rejected At:</b> <code>${new Date().toLocaleString()}</code>

<i>Request rejected. User will NOT receive any signals or trades.</i>
`.trim();

      const updatedKeyboard: TelegramInlineKeyboard = {
        inline_keyboard: [
          [
            { text: "✅ APPROVE NOW", callback_data: `adm:req:approve:${targetUserId}` },
            { text: "🚫 BLOCK USER", callback_data: `adm:req:block:${targetUserId}` },
          ],
          [
            { text: "👥 All Users", callback_data: "adm:users:menu" },
      ...
      await sendSingleTelegramMessage(
        user.chatId || targetUserId,
        `❌ <b>ACCESS REQUEST REJECTED</b>\n━━━━━━━━━━━━━━━━━━━\nYour Telegram Bot access request (ID: <code>${user.userId}</code>) was rejected by the Super Admin.\n\n<i>You are not authorized to receive trade signals. Please contact the Super Admin if you believe this is an error.</i>`
      ).catch(() => {});
      return;
    }

    if (data.startsWith("adm:req:block:")) {
      const targetUserId = data.replace("adm:req:block:", "").trim();
      const userKey = Object.keys(telegramUsersStore).find(
        (k) => telegramUsersStore[k].userId === targetUserId || telegramUsersStore[k].chatId === targetUserId
      );
      const user = userKey ? telegramUsersStore[userKey] : null;
      ...
      superAdminService.logAction(
        "USER_BLOCKED_TELEGRAM",
        `Blocked user ${user.firstName} (${targetUserId}) via Telegram One-Tap button`,
        cbUserId,
        targetUserId
      );

      await answerTelegramCallback(cbId, `🚫 Blocked user ${user.firstName}. Signals halted immediately.`, false);

      const updatedText = `
🚫 <b>USER ACCOUNT BLOCKED</b>
━━━━━━━━━━━━━━━━━━━
<b>👤 User:</b> <b>${user.firstName || "Trader"} ${user.lastName || ""}</b> (${user.username || "No @username"})
<b>🆔 Telegram ID:</b> <code>${user.userId}</code>
<b>🔒 Status:</b> 🚫 <code>BLOCKED</code>
<b>🕒 Blocked At:</b> <code>${new Date().toLocaleString()}</code>

<i>User is blocked. Trade signal delivery and bot interactions have been stopped immediately.</i>
`.trim();

      const updatedKeyboard: TelegramInlineKeyboard = {
        inline_keyboard: [
          [
            { text: "✅ UNBLOCK & APPROVE", callback_data: `adm:req:approve:${targetUserId}` },
          ],
          [
            { text: "👥 All Users", callback_data: "adm:users:menu" },
          ],
      ...
      await sendSingleTelegramMessage(
        user.chatId || targetUserId,
        `🚫 <b>ACCOUNT BLOCKED</b>\n━━━━━━━━━━━━━━━━━━━\nYour Telegram account (ID: <code>${user.userId}</code>) has been blocked from GMC Trading AI signals and bot commands by the Super Admin.`
      ).catch(() => {});
      return;
    }

    if (data.startsWith("adm:req:revoke:")) {
      const targetUserId = data.replace("adm:req:revoke:", "").trim();
      const userKey = Object.keys(telegramUsersStore).find(
        (k) => telegramUsersStore[k].userId === targetUserId || telegramUsersStore[k].chatId === targetUserId
      );
      const user = userKey ? telegramUsersStore[userKey] : null;
      ...
      superAdminService.logAction(
        "USER_REVOKED_TELEGRAM",
        `Revoked user ${user.firstName} (${targetUserId}) to PENDING`,
        cbUserId,
        targetUserId
      );

      await answerTelegramCallback(cbId, `⚠️ Access placed back in PENDING for ${user.firstName}.`, false);

      const reqView = superAdminService.renderUserAccessRequest(user);
      await editTelegramMessageText(cbChatId, cbMsgId, reqView.text, reqView.keyboard);

      await sendSingleTelegramMessage(
        user.chatId || targetUserId,
        `⚠️ <b>ACCESS REVOKED</b>\n━━━━━━━━━━━━━━━━━━━\nYour Telegram Bot access has been placed back in PENDING status by the Super Admin.`
      ).catch(() => {});
      return;
    }

    if (data.startsWith("adm:user:view:")) {
      const targetUserId = data.replace("adm:user:view:", "").trim();
      const user = Object.values(telegramUsersStore).find((u) => u.userId === targetUserId || u.chatId === targetUserId);
      if (user) {
    ...
    if (data.startsWith("adm:usr:bot:")) {
      const parts = data.split(":");
      const targetUserId = parts[3];
      const botKey = (parts[4] || "all") as "all" | "khatarnak" | "harami" | "war_room";
      ...
      if (user) {
        ...
        superAdminService.logAction(
          "BOT_ACCESS_CHANGED",
          `Changed bot access to ${botLabel} for user ${user.firstName} (${targetUserId})`,
          cbUserId,
          targetUserId
        );

        await answerTelegramCallback(cbId, `🤖 Bot access configured: ${botLabel}`, false);

        // Notify user about bot selection
        await sendSingleTelegramMessage(
          user.chatId || targetUserId,
          `🤖 <b>BOT ACCESS CONFIGURATION UPDATED</b>\n━━━━━━━━━━━━━━━━━━━━\nHello <b>${user.firstName}</b>!\nYour signal feed has been configured by Super Admin to: <b>${botLabel}</b>.\n\n<i>⚡ You will now receive all live trades and real-time execution updates corresponding to your assigned bot feed.</i>`
        ).catch(() => {});
    ...
    if (data === "adm:delivery:menu") {
      ...
      for (const log of serverDeliveryLogs) {
        totalSent += log.recipientsCount;
        totalSuccess += log.successCount;
      }
      const successRate = totalSent > 0 ? (totalSuccess / totalSent) * 100 : 100;

      const isKillSwitch = superAdminService.getConfig().masterStatus === "KILL_SWITCH";

      const deliveryView = superAdminService.renderDeliveryCenterMenu({
        totalSignals: serverDeliveryLogs.length,
        activeSubscribers: approvedUsers.length,
        successRate,
        recentDeliveries: serverDeliveryLogs,
        failedDeliveries: serverDeliveryFailures,
        isKillSwitch,
      });

      await editTelegramMessageText(cbChatId, cbMsgId, deliveryView.text, deliveryView.keyboard);
      return;
    }
    ...
    if (data.startsWith("adm:usr:block:")) {
      const targetUserId = data.replace("adm:usr:block:", "").trim();
      const userKey = Object.keys(telegramUsersStore).find((k) => telegramUsersStore[k].userId === targetUserId || telegramUsersStore[k].chatId === targetUserId);
      const user = userKey ? telegramUsersStore[userKey] : null;
      if (user) {
        user.status = "blocked";
        ...
        sendSingleTelegramMessage(
          user.chatId || targetUserId,
          `🚫 <b>Access Blocked</b>\n━━━━━━━━━━━━━━━━━━━━\nYour access to the GMC Trading AI Bot has been blocked by the Super Admin. Signal delivery has been stopped immediately.`
        ).catch(() => {});
        const card = superAdminService.renderUserCard(user);
        await editTelegramMessageText(cbChatId, cbMsgId, card.text, card.keyboard);
      }
    ...
    if (data.startsWith("adm:usr:unblock:")) {
      ...
      if (user) {
        user.status = "approved";
        user.decisionAt = new Date().toISOString();
        if (user.approvalDuration === "lifetime" || user.planType === "lifetime" || !user.approvalDuration) {
          user.approvalDuration = "lifetime";
          user.approvalDurationLabel = "Lifetime (Permanent Access)";
          user.planType = "lifetime";
          user.expiresAt = null;
        ...
        sendSingleTelegramMessage(
          user.chatId || targetUserId,
          `✅ <b>ACCOUNT UNBLOCKED & RESTORED</b>\n━━━━━━━━━━━━━━━━━━━━\nHello <b>${user.firstName}</b>!\nYour access to the GMC Trading AI Bot has been unblocked by the Super Admin. You are now authorized to receive live trading signals 24/7.`
        ).catch(() => {});
        const card = superAdminService.renderUserCard(user);
        await editTelegramMessageText(cbChatId, cbMsgId, card.text, card.keyboard);
      }
    ...
    if (data.startsWith("adm:usr:revoke:")) {
      const targetUserId = data.replace("adm:usr:revoke:", "").trim();
      const userKey = Object.keys(telegramUsersStore).find((k) => telegramUsersStore[k].userId === targetUserId || telegramUsersStore[k].chatId === targetUserId);
      const user = userKey ? telegramUsersStore[userKey] : null;
      if (user) {
        user.status = "pending";
        user.decisionAt = new Date().toISOString();
        saveTelegramUsers();
        superAdminService.logAction("USER_ACCESS_REVOKED", `Revoked access for user ${user.firstName} (${targetUserId}) to PENDING`, cbUserId, targetUserId);
        sendSingleTelegramMessage(
          user.chatId || targetUserId,
          `⚠️ <b>Access Revoked</b>\n━━━━━━━━━━━━━━━━━━━━\nYour subscription to GMC Trading AI signals has been placed back in PENDING status by the Super Admin.`
        ).catch(() => {});
        const card = superAdminService.renderUserCard(user);
        await editTelegramMessageText(cbChatId, cbMsgId, card.text, card.keyboard);
      }
    ...
    if (data.startsWith("adm:trd:appr:")) {
      ...
      if (candidate) {
        ...
        if (mt5Config.telegramSignalsEnabled) {
          const sent = await sendServerTelegramMessage(signalMsg, undefined, undefined, `${candidate.id}_NEW_SETUP`, true);
          if (sent && serverActiveTrade) {
            ...
            tradeStateManager.markSignalDispatched(candidate.id);
          }
        }
...
`.trim();
                  ...
                  // Super Admin /stop, /kill, /pause -> Emergency Halt All Signal Broadcasts
                  if (textLower.startsWith("/stop") || textLower.startsWith("/kill") || textLower.startsWith("/pause") || ["stop", "pause", "kill"].includes(textLower)) {
                    superAdminService.getConfig().masterStatus = "KILL_SWITCH";
                    superAdminService.saveConfig();
                    advancedRiskManager.setRiskMode("EMERGENCY");
                    superAdminService.logAction("KILL_SWITCH_ENGAGED", `Emergency signal halt invoked by ${userId}`, userId);
                    await sendSingleTelegramMessage(
                      chatId,
                      `🚨 <b>EMERGENCY KILL SWITCH ACTIVATED (/pause)</b>\n━━━━━━━━━━━━━━━━━━━━\n<b>STATUS:</b> <code>ALL SIGNALS PAUSED</code>\n<b>AUTO RISK MODE:</b> <code>EMERGENCY</code>\n\nAll automated signal broadcasts have been HALTED immediately across all connected bots & subscribers.\n\n<i>To resume, send /resume or tap ▶️ Start Signals in /admin.</i>`
                    );
                    continue;
                  }

                  // Super Admin /resume, /startsignals -> Resume Signal Broadcast
                  if (textLower.startsWith("/resume") || textLower.startsWith("/startsignals") || ["resume", "startsignals"].includes(textLower)) {
                    superAdminService.getConfig().masterStatus = "RUNNING";
                    superAdminService.saveConfig();
                    advancedRiskManager.setRiskMode("NORMAL");
                    superAdminService.logAction("BROADCAST_RESUMED", `Signal broadcast resumed by ${userId}`, userId);
                    await sendSingleTelegramMessage(
                      chatId,
                      `🟢 <b>SIGNAL BROADCAST RESUMED (/resume)</b>\n━━━━━━━━━━━━━━━━━━━━\n<b>STATUS:</b> <code>ONLINE & SCANNING 24/7</code>\n<b>AUTO RISK MODE:</b> <code>NORMAL</code>\n\nMaster signal generator is now ONLINE and broadcasting live trades to all approved subscribers in real time.`
                    );
                    continue;
                  }

                  // Super Admin /riskmode -> Control Auto Risk Modes (NORMAL / SAFE / EMERGENCY)
                  if (textLower.startsWith("/riskmode") || textLower.startsWith("/mode")) {
                    ...
                    if (subMode === "NORMAL" || subMode === "SAFE" || subMode === "EMERGENCY") {
                      const res = advancedRiskManager.setRiskMode(subMode as any);
                      if (subMode === "EMERGENCY") {
                        superAdminService.getConfig().masterStatus = "KILL_SWITCH";
                      } else {
                        superAdminService.getConfig().masterStatus = "RUNNING";
                      }
                      ...
                      await sendSingleTelegramMessage(
                        chatId,
                        `⚙️ <b>AUTO RISK MODE SWITCHED TO ${res.mode}</b>\n━━━━━━━━━━━━━━━━━━━━\n<b>Description:</b> <i>${res.description}</i>\n<b>Trade Lock Status:</b> <code>${res.isLocked ? "ON (BLOCKED)" : "OFF (ARMED)"}</code>\n\n<i>System synchronized in real-time across Dashboard & Telegram.</i>`
                      );
                    } else {
                      const curMode = advancedRiskManager.getRiskMode();
                      const lockReport = tradeStateManager.getTradeLockStatusReport();
                      await sendSingleTelegramMessage(
                        chatId,
                        `⚙️ <b>AUTO RISK MODE CONTROL</b>\n━━━━━━━━━━━━━━━━━━━━\n<b>Current Mode:</b> <code>${curMode}</code>\n<b>Trade Lock:</b> <code>${lockReport.lockStatusLabel}</code>\n\n<b>Commands:</b>\n• <code>/riskmode normal</code> — Full signal generation\n• <code>/riskmode safe</code> — Highest confidence setups only (≥92%)\n• <code>/riskmode emergency</code> — Block all new signals immediately\n• <code>/pause</code> — Fast emergency halt\n• <code>/resume</code> — Resume normal trading`
                      );
                    }
                    continue;
                  }

                  // Super Admin /lock or /tradelock -> Trade Lock Status Dashboard
                  if (textLower.startsWith("/lock") || textLower.startsWith("/tradelock")) {
                    const lockReport = tradeStateManager.getTradeLockStatusReport();
                    ...
                    const activeDetails = act
                      ...
                      : `<code>NONE (Scanning 24/7)</code>`;

                    await sendSingleTelegramMessage(
                      chatId,
                      `🔒 <b>TRADE LOCK STATUS DASHBOARD</b>\n━━━━━━━━━━━━━━━━━━━━\n<b>🛡️ LOCK STATUS:</b> <code>${lockReport.lockStatusLabel}</code>\n<b>⚡ AUTO RISK MODE:</b> <code>${lockReport.autoRiskMode}</code>\n<b>📊 CURRENT ACTIVE TRADE:</b> ${activeDetails}\n<b>🆔 CURRENT TRADE ID:</b> <code>${lockReport.currentTradeId}</code>\n<b>⏳ NEXT ALLOWED SIGNAL:</b> <code>${lockReport.nextAllowedSignalTimeFormatted}</code>\n<b>⛔ PRIMARY BLOCK REASON:</b>\n<i>${lockReport.primaryBlockReason}</i>\n\n<b>📅 DAILY LIMIT:</b> <code>${lockReport.daily.tradesExecuted} / ${lockReport.daily.limit} trades</code>\n<b>🛑 CONSECUTIVE LOSSES:</b> <code>${lockReport.consecutiveLoss.count} / ${lockReport.consecutiveLoss.limit} SLs</code>\n<b>⏲️ COOLDOWN TIMER:</b> <code>${lockReport.cooldown.displayText}</code>`
                    );
                    continue;
                  }

                  // Super Admin /logs or /rejections -> Show Signal Rejection Logs
                  if (textLower.startsWith("/logs") || textLower.startsWith("/rejections") || textLower.startsWith("/rejects")) {
                    const logs = advancedRiskManager.getRejectionLogs(7);
                    if (logs.length === 0) {
                      await sendSingleTelegramMessage(
                        chatId,
                        `📋 <b>SIGNAL REJECTION LOGS</b>\n━━━━━━━━━━━━━━━━━━━━\n<i>No rejected signals recorded. All signals are evaluating within clean risk boundaries.</i>`
                      );
                    } else {
                      ...
                      await sendSingleTelegramMessage(
                        chatId,
                        `📋 <b>SIGNAL REJECTION LOGS (LAST ${logs.length})</b>\n━━━━━━━━━━━━━━━━━━━━\n${lines}\n\n<i>Use /riskmode to adjust system sensitivity.</i>`
                      );
                    }
                    continue;
                  }
                  ...
                  // Super Admin /ais or /brains -> Open 4-AI Control Hub
                  if (textLower.startsWith("/ais") || textLower.startsWith("/brains") || ["ais", "brains"].includes(textLower)) {
                    const menu = superAdminService.renderAiSystemsControlMenu();
                    await sendSingleTelegramMessage(chatId, menu.text, undefined, menu.keyboard);
                    continue;
                  }

                  // Super Admin /active or /currenttrade -> Open Active Setup Monitor
                  if (textLower.startsWith("/active") || textLower.startsWith("/currenttrade") || ["active", "currenttrade"].includes(textLower)) {
                    const csmState = centralSignalManager.getState(liveGold);
                    const activeSetup = csmState.activeSetup || (serverActiveTrade ? {
                      setupId: serverActiveTrade.signalId || serverActiveTrade.id,
                      brainName: serverActiveTrade.isWarRoomUpgraded ? "War Room Supreme" : "Harami AI Master",
                  ...
                  // Super Admin /trace or /audit -> Open Decision Trace Logs
                  if (textLower.startsWith("/trace") || textLower.startsWith("/audit") || ["trace", "audit"].includes(textLower)) {
                    const logs = centralSignalManager.getAuditLogs();
                    const menu = superAdminService.renderDecisionTraceView(logs);
                    await sendSingleTelegramMessage(chatId, menu.text, undefined, menu.keyboard);
                    continue;
                  ...
                  // Super Admin /history -> Open Signal History
                  if (textLower.startsWith("/history") || ["history", "signals"].includes(textLower)) {
                    const logs = centralSignalManager.getAuditLogs();
                    const historyItems = logs
                      .filter((l) => l.eventType === "SETUP_ACTIVATED" || l.eventType === "TP_HIT" || l.eventType === "SL_HIT" || l.eventType === "SETUP_CLOSED")
                      .map((l) => ({
                    ...
                    const menu = superAdminService.renderSignalHistoryView("ALL", historyItems);
                    await sendSingleTelegramMessage(chatId, menu.text, undefined, menu.keyboard);
                    continue;
                  }

                  // Super Admin /market or /gold -> Open Live Market Status
                  if (textLower.startsWith("/market") || textLower.startsWith("/gold") || ["market", "gold"].includes(textLower)) {
                    const goldTick = fcsMarketService.getLiveTick("XAUUSD");
                    const goldData = {
                      price: goldTick?.price || 4495.50,
                      bid: goldTick?.bid || 4495.35,
...
`.trim();
                  ...
                  if (textLower.startsWith("/trade_now") || textLower.startsWith("/force_trade") || textLower.startsWith("/forcesignal") || textLower.startsWith("/generate_signal")) {
                    ...
                    const isBuy = Math.sin(entry * 2.5) >= 0;
                    ...
                    const reason = generateDynamicReason(direction, now);
                    ...
                    const tradeObj = {
                      ...
                      direction,
                      entry,
                      actualExecutedEntryPrice: entry,
                      ...
                      confidence,
                      reason,
                      status: "ENTRY_CONFIRMED" as const,
                      currentBid: tick.bid,
                      currentAsk: tick.ask,
                      ...
                      auditLogs: [
                        {
                          timestamp: nowUtc,
                          event: "ADMIN_TRIGGERED_SIGNAL",
                          price: entry,
                          bid: tick.bid,
                          ask: tick.ask,
                          note: `Super Admin generated instant verified ${direction} signal at $${entry}`,
                        },
                      ],
                    };

                    serverActiveTrade = tradeObj as any;
                    serverCurrentDecision = direction;
                    serverLastSignalTime = now;
                    serverLastDispatchedSignal = { direction, entry, timestamp: now };

                    centralSignalManager.registerOrBroadcastSetup("HARAMI_AI", {
                      setupId: signalId,
                      assetKey: "XAUUSD",
                      timeframe: "15M",
                      direction,
                    ...
                    const sent = await sendServerTelegramMessage(signalText, undefined, chartBuffer, `${signalId}_NEW_SETUP`, true);
                    if (sent && serverActiveTrade) {
                      serverActiveTrade.entryDispatched = true;
                      if (!serverActiveTrade.dispatchedOutcomes.includes("SIGNAL")) {
                        serverActiveTrade.dispatchedOutcomes.push("SIGNAL");
                ...
                if (user.status === "blocked") {
                  await sendSingleTelegramMessage(
                    chatId,
                    `🚫 <b>Access Blocked</b>\n━━━━━━━━━━━━━━━━━━━\nYour Telegram account (ID: <code>${userId}</code>) has been blocked from GMC Trading AI Bot by the Super Admin.\n\n<i>Trade signals and bot execution have been halted for this account. Contact the Super Admin for unblock requests.</i>`
                  );
                  continue;
                }
                ...
                if (textLower.startsWith("/lifeline") || textLower.startsWith("/run") || textLower.startsWith("/bot") || textLower.startsWith("/myaccess") || textLower.startsWith("/account")) {
                  ...
                  replyText = `
<b>🤖 GMC TRADING AI • BOT RUN & LIFELINE</b>
━━━━━━━━━━━━━━━━━━━
...
`.trim();
  ...
  function checkSignalDuplicate(
    originEngine: "HARAMI_AI" | "WAR_ROOM" | "KHATARNAK_JUGAAD" | "KHATARNAK" | "RETEST_X" | string,
    direction: string,
    price: number,
    ignoreTradeId?: string
  ): boolean {
    const now = Date.now();
    const DEDUPLICATION_WINDOW_MS = 3 * 60 * 1000; // 3 minutes
    const PRICE_TOLERANCE = 0.80; // $0.80 price proximity

    const cleanIgnoreId = ignoreTradeId ? String(ignoreTradeId).replace("_NEW_SETUP", "").replace("#", "").trim().toUpperCase() : "";

    // RULE 1 & RULE 3: ONLY ONE ACTIVE TRADE AT A TIME & NO OPPOSITE SIGNALS
    // If ANY trade is active in ANY engine, BLOCK ALL NEW SIGNALS (BUY or SELL) unless it's the SAME trade being dispatched
    if (serverActiveTrade && serverActiveTrade.status !== "CLOSED" && serverActiveTrade.status !== "CANCELLED") {
      const activeId = String(serverActiveTrade.signalId || serverActiveTrade.id || "").replace("#", "").trim().toUpperCase();
      ...
      if (!isSame) {
        console.log(`[DEDUP/SINGLE-TRADE GUARD]: Blocked ${direction} signal from ${originEngine}. Active Harami trade (#${activeId}, ${serverActiveTrade.direction}) is currently running.`);
        return true;
      }
    }
    ...
    if (tradeStateManager.hasActiveTrade()) {
      ...
      const activeId = String(active?.signalId || active?.id || "").replace("#", "").trim().toUpperCase();
      const isSame = cleanIgnoreId && activeId && (cleanIgnoreId === activeId || cleanIgnoreId.includes(activeId) || activeId.includes(cleanIgnoreId));
      if (!isSame) {
        console.log(`[DEDUP/SINGLE-TRADE GUARD]: Blocked ${direction} signal from ${originEngine}. Unified active trade (#${activeId}, ${active?.direction}) is currently running.`);
        return true;
      }
    }
    ...
    if (
      ...
      const activeId = String(activeWarRoomSetup.setupId || "").replace("#", "").trim().toUpperCase();
      const isSame = cleanIgnoreId && activeId && (cleanIgnoreId === activeId || cleanIgnoreId.includes(activeId) || activeId.includes(cleanIgnoreId));
      if (!isSame) {
        console.log(`[DEDUP/SINGLE-TRADE GUARD]: Blocked ${direction} signal from ${originEngine}. Active War Room setup (#${activeId}) is currently running.`);
        return true;
      }
    }

    if (serverActiveKhatarnakSetup) {
      const activeId = String(serverActiveKhatarnakSetup.id || "").replace("#", "").trim().toUpperCase();
      const isSame = cleanIgnoreId && activeId && (cleanIgnoreId === activeId || cleanIgnoreId.includes(activeId) || activeId.includes(cleanIgnoreId));
      if (!isSame) {
        console.log(`[DEDUP/SINGLE-TRADE GUARD]: Blocked ${direction} signal from ${originEngine}. Active Khatarnak setup (#${activeId}) is currently running.`);
        return true;
      }
    }
    ...
    if (centralActive) {
      const activeId = String(centralActive.setupId || "").replace("#", "").trim().toUpperCase();
      const isSame = cleanIgnoreId && activeId && (cleanIgnoreId === activeId || cleanIgnoreId.includes(activeId) || activeId.includes(cleanIgnoreId));
      if (!isSame) {
    ...
    // Cooldown check for newly candidate setups (skip if we are dispatching an existing active trade)
    if (!cleanIgnoreId) {
      const cooldown = tradeStateManager.checkCooldown();
  ...
  async function sendServerTelegramMessage(
    ...
    if (dedupCheck.isDuplicate) {
      console.log(`[SERVER IDEMPOTENCY GUARD]: Suppressed duplicate server broadcast: ${dedupCheck.reason}`);
  ...
  async function sendServerTelegramMessageInternal(
    ...
    if (overrideChatId) {
      ...
      if (ok) {
        serverTelegramIdempotency.markDispatched(customAlertId, text, overrideChatId);
      }
      return ok;
    }
    ...
    // Emergency Kill Switch Guard Check
    const superAdminCfg = superAdminService.getConfig();
    if (superAdminCfg.masterStatus === "KILL_SWITCH") {
      console.warn("[KILL SWITCH ACTIVE]: Public subscriber broadcast halted by Super Admin Emergency Stop.");
      superAdminService.logAction(
        "KILL_SWITCH_BLOCKED_BROADCAST",
        `Emergency Kill Switch halted trade signal broadcast.`,
        "SYSTEM"
      );
      // Alert master admin only
      await sendSingleTelegramMessage(
        masterId,
        `🚨 <b>SIGNAL NOT BROADCASTED: KILL SWITCH ACTIVE</b>\n━━━━━━━━━━━━━━━━━━━━\nA trade signal was generated but dispatch to subscribers was halted because the Emergency Kill Switch is ON.\n\n<i>Use /admin → Resume Broadcast to re-enable live trade delivery.</i>`
      ).catch(() => {});
    ...
    let signalEngine: string = "HARAMI_AI";
    ...
    } else if (text.includes("HARAMI") || (customAlertId && customAlertId.startsWith("HA-")) || (customAlertId && customAlertId.startsWith("HRM-"))) {
    ...
    if ((signalEngine as any) !== "HARAMI_AI" && (signalEngine as any) !== "GMC_SYSTEM") {
    ...
    const sourceEngineMap: Record<string, AiBrainSource> = {
      WAR_ROOM: "WAR_ROOM",
      KHATARNAK: "KHATARNAK_JUGAAD",
      HARAMI_AI: "HARAMI_AI",
      PRECISION_HUNTER: "PRECISION_HUNTER",
    };
    const mappedSource = sourceEngineMap[signalEngine];
    if (mappedSource && !centralSignalManager.isAiSourceEnabled(mappedSource)) {
      console.warn(`[CENTRAL GATEKEEPER STRICT ENFORCEMENT]: Dropped Telegram broadcast. Source '${botLabel}' is DISABLED in Central Orchestrator.`);
      serverTelegramDeliveryStatus = "Idle";
      return false;
    }
    ...
    // User Directive:
    // 1. System must first send the complete trade signal (BUY/SELL, entry, SL, TP)
    //    before sending any TP hit, SL hit, breakeven, trailing SL, or profit update.
    // 2. Never generate TP/SL updates for a trade that was not previously opened.
    // 3. Maintain a trade ID and only manage SL/TP after the entry signal is confirmed.
    ...
    // Force isEntryDispatch = true if customAlertId ends with _NEW_SETUP or _ENTRY or explicitly set
    const isEntryDispatchFinal = Boolean(
      isEntryDispatch ||
      (customAlertId && (customAlertId.endsWith("_NEW_SETUP") || customAlertId.endsWith("_ENTRY")))
    );
    ...
    // GLOBAL COOLDOWN + QUALITY GATE CHECK (Only applies to new unapproved trades, NOT entry dispatches of existing trades)
    if (!isEntryDispatchFinal) {
...
`.trim();
    ...
    // 3. Evaluate and dispatch to ALL registered users in telegramUsersStore
    for (const [userIdKey, u] of Object.entries(telegramUsersStore)) {
      ...
      if (!cleanUid || !targetChatId) continue;

      if (dispatchedRecipients.has(cleanUid) || dispatchedRecipients.has(targetChatId)) {
        // Master ID, channel, or already dispatched recipient in this batch
        continue;
      }
      ...
      if (isAccessValid && targetChatId) {
        dispatchedRecipients.add(cleanUid);
        dispatchedRecipients.add(targetChatId);
    ...
    if (successCount > 0) {
      ...
      if (isNewTradeSetup) {
        ...
        console.log(`[ENTRY CONFIRMED]: Initial signal for trade #${signalIdExtracted} broadcasted. SL/TP management is now ACTIVE.`);
      }
  ...
  class GoldMarketDataService {
    ...
    public async pollQuote(): Promise<LiveGoldTick> {
      ...
      try {
        ...
        // 3. Resilient fallback to FCS Live Market Tick if REST providers timed out
        if (primaryPrice === null) {
          const fcsTick = fcsMarketService.getLiveTick("XAUUSD");
          if (fcsTick && fcsTick.price > 1800) {
            primaryPrice = Number(fcsTick.price.toFixed(2));
            primaryHigh = fcsTick.high24h || primaryPrice * 1.004;
            primaryLow = fcsTick.low24h || primaryPrice * 0.996;
            primaryChange = fcsTick.change24h || 0;
            primaryChangePct = fcsTick.changePercent24h || 0;
            primaryTimestamp = now;
            primaryProvider = "FALLBACK";
            activeProviderName = "FCS Market Multi-Feed";
          }
        }
        ...
        console.warn("[GOLD MARKET DATA POLLER ERROR]:", pollErr);
      } finally {
  ...
  function isMarketOpen(): boolean {
    ...
    // Saturday (6): Standard Gold Spot closed
...
`.trim();
  ...
  async function ensureTradeSignalDispatched(trade: any): Promise<boolean> {
    ...
    const entryHigh = Array.isArray(trade.entryZone) ? trade.entryZone[1] : Number((entry + 0.5).toFixed(2));
    const isGold = (trade.symbol || "XAUUSD").includes("XAU") || (trade.symbol || "XAUUSD").includes("GOLD");
    const isBuy = direction === "BUY";
    let sl = trade.sl || trade.stopLoss;
    let tp1 = trade.tp1;
    let tp2 = trade.tp2;
    let tp3 = trade.tp3;
    let tp4 = trade.tp4;
    ...
    if (isGold) {
      ...
      tp3 = isBuy ? Number((entry + 12.00).toFixed(2)) : Number((entry - 12.00).toFixed(2));
      tp4 = isBuy ? Number((entry + 15.00).toFixed(2)) : Number((entry - 15.00).toFixed(2));
    } else {
      sl = sl || (isBuy ? Number((entry - 4.5).toFixed(2)) : Number((entry + 4.5).toFixed(2)));
      tp1 = tp1 || (isBuy ? Number((entry + 7.0).toFixed(2)) : Number((entry - 7.0).toFixed(2)));
      tp2 = tp2 || (isBuy ? Number((entry + 10.0).toFixed(2)) : Number((entry - 10.0).toFixed(2)));
    ...
    if (ok) {
      if (!trade.dispatchedOutcomes.includes("SIGNAL")) {
        trade.dispatchedOutcomes.push("SIGNAL");
      }
  ...
  async function executeServerSignalEngineTick() {
    ...
    // 0. WEEKEND / MARKET CLOSED PROTECTION (Allows continuous live feed if active)
    const tick = await fetchLiveServerGoldTick();
    ...
    if (!systemActiveTrade) {
      ...
      if (
        ...
        if (buyScore >= sellScore && buyScore >= targetConfidence) {
          direction = "BUY";
          confidence = buyScore;
        } else if (sellScore > buyScore && sellScore >= targetConfidence) {
          direction = "SELL";
          ...
          if (checkBid >= trade.tp1 && !trade.tp1Hit) {
            ...
            if (!trade.dispatchedOutcomes.includes(outcomeKey)) {
              ...
              if (mt5Config.closeModeOnTarget === "close_all") {
                ...
                serverAccountBalance += pnl;
                mt5AccountMetrics.balance += pnl;
                mt5AccountMetrics.equity = mt5AccountMetrics.balance;
                mt5AccountMetrics.dailyPnL += pnl;
                mt5AccountMetrics.totalProfit += pnl;
  ...
  async function start247ServerSignalEngine() {
    ...
    serverKhatarnakLifecycleEvents.clear();
    ...
    while (true) {
      try {
        ...
        if (livePx) {
          ...
          if (centralSignalManager.isAiSourceEnabled("KHATARNAK_JUGAAD")) {
      ...
      // High-precision setup detection polling every 3 seconds
      await new Promise((r) => setTimeout(r, 3000));
    }
  }
  ...
  app.post("/api/telegram/trigger-signal", async (req, res) => {
    try {
      ...
      const sent = await sendServerTelegramMessage(signalText, undefined, chartBuffer, `${serverActiveTrade.id}_NEW_SETUP`, true);
      if (sent && serverActiveTrade) {
        serverActiveTrade.entryDispatched = true;
        if (!serverActiveTrade.dispatchedOutcomes.includes("SIGNAL")) {
          serverActiveTrade.dispatchedOutcomes.push("SIGNAL");
        }
        tradeStateManager.markSignalDispatched(serverActiveTrade.id);
      }
...
`.trim();
  ...
  app.post("/api/telegram/send", async (req, res) => {
    try {
      ...
      const dedupCheck = serverTelegramIdempotency.isDuplicate(alertId, text, targetChatId);
      if (dedupCheck.isDuplicate) {
        console.log(`[API TELEGRAM DEDUP GUARD]: Blocked duplicate dispatch: ${dedupCheck.reason}`);
        return res.json({
          ok: true,
          duplicateSuppressed: true,
          message: dedupCheck.reason,
          key: dedupCheck.key,
        });
      }
  ...
  app.get("/api/telegram/idempotency/stats", (req, res) => {
    res.json({ ok: true, stats: serverTelegramIdempotency.getStats() });
  });

  app.post("/api/telegram/idempotency/reset", (req, res) => {
    serverTelegramIdempotency.resetRegistry();
    res.json({ ok: true, message: "Idempotency registry reset successfully." });
  });
...
<i>Automated institutional trade execution active. Managed strictly via GMC AI War Room.</i>`;
  ...
  // 2. Toggle AI Source ON / OFF (Harami AI, Khatarnak Jugaad, War Room, Precision Hunter)
  app.post("/api/central-signal-manager/toggle-ai", (req, res) => {
    try {
      ...
      } else if (brainSource === "PRECISION_HUNTER") {
        cfg.precisionHunterEnabled = isEnabled;
      }
      superAdminService.saveConfig();
  ...
  app.post("/api/central-signal-manager/config", (req, res) => {
    try {
      const { minScoreThreshold, cooldownMinutes, autoBroadcastToTelegram, haramiEnabled, khatarnakEnabled, warRoomEnabled, precisionHunterEnabled } = req.body || {};
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`
=====================================================
🚀 [SERVER ONLINE]: GMC Trading Autonomous Platform Running on Port ${PORT}
🤖 [TELEGRAM WORKER ONLINE]: Long-Polling Background Daemon Connected (Bot: @GMCtelegrambot)
...
// Global Process Crash Guards: Prevent unexpected uncaught exceptions from killing the 24/7 server
process.on("uncaughtException", (err) => {
  console.error("[CRITICAL UNCAUGHT EXCEPTION RECOVERED]:", err);
});

process.on("unhandledRejection", (reason) => {
  console.error("[CRITICAL UNHANDLED REJECTION RECOVERED]:", reason);
});
